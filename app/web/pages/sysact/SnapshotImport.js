...
import { snapshotImport } from '../../service/sysact';
      ...
      if (!err) {
        Modal.confirm({
          ...
          onOk: () => {
            ...
            const to_part_id = values['to_part_id'].trim();
            ...
            snapshotImport({ from_guids, from_part_ids, to_guids, keys, to_part_id }).then(data => {
              if (data.code === 0) {
    ...
    return (
      ...
      <Card>
        <Form {...formItemLayout} onSubmit={this.handleSubmit} style={{ marginTop: 40 }}>
          ...
          <Form.Item label="内网要导入的机器">
            {getFieldDecorator('to_part_id', {
              rules: [{ required: true, message: '不能为空' }]
            })(
              <Input />
            )}
