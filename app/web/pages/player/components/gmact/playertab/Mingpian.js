...
class Mingpian extends React.Component {
  ...
  handleSubmit1 = (e) => {
    ...
    this.props.form.validateFieldsAndScroll(['value_add'], (err, values) => {
      if (!err) {
        ...
        Modal.confirm({
          title: '确认操作',
          content: `添加名片: ${values['value_add']}`,
          onOk: () => {
            this.setState({ loading1: true });
            changePlayerData({ type: 4, new_value: parseInt(values['value_add']), guid, part_id }).then(data => {
              if (data.code === 0) {
  ...
  handleSubmit2 = (e) => {
    ...
    this.props.form.validateFieldsAndScroll(['value_del'], (err, values) => {
      if (!err) {
        ...
        Modal.confirm({
          title: '确认操作',
          content: `删除名片: ${values['value_del']}`,
          onOk: () => {
            this.setState({ loading2: true });
            changePlayerData({ type: 4, new_value: -parseInt(values['value_del']), guid, part_id }).then(data => {
              if (data.code === 0) {
                message.success('操作成功');
              } else {
                message.error('操作失败');
...
export default Form.create()(Mingpian);
