...
import { setPetLflevel } from '../../../../../service/gmact';

/**
 * 修改宠物等级
 */
@Form.create()
class LfLevel extends React.PureComponent {
  ...
  handleSubmit = (e) => {
    ...
    this.props.form.validateFieldsAndScroll((err, values) => {
      if (!err) {
        ...
        Modal.confirm({
          title: '确认操作',
          content: `设置宠物 [${values['pet_guid']}] 对应炼符等级为: ${values['level']}`,
          onOk: () => {
            this.setState({ loading: true });
            setPetLflevel({ ...values, guid, part_id }).then(data => {
              if (data.code === 0) {
